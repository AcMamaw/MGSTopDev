...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            DB::transaction(function() use ($validated, &$order, &$payment) {
                ...
                if ($firstInventory) {
                    if (!empty($firstInventory->product_type)) {
                        ...
                        if ($validated['product_type'] === 'stockin_id' && $firstInventory->stockin && !empty($firstInventory->stockin->type)) {
                            $orderProductTypeText = $firstInventory->stockin->type;
                        } elseif ($validated['product_type'] === 'deliverydetails_id' && $firstInventory->deliveryDetail && !empty($firstInventory->deliveryDetail->type)) {
                ...
                foreach ($validated['items'] as $it) {
                    ...
                    if ($validated['product_type'] === 'stockin_id' && empty($inventory->stockin_id)) {
                        throw new \Exception("Selected product '{$inventory->product->product_name}' is not a Ready Made item");
                    }
                    if ($validated['product_type'] === 'deliverydetails_id' && empty($inventory->deliverydetails_id)) {
                        throw new \Exception("Selected product '{$inventory->product->product_name}' is not a Customized item");
                    }
            ...
            return response()->json([
                ...
                'order' => $order->load('items', 'customer', 'payments'), // adjust relation names if needed
                'payment' => $payment,
            ]);
         ...
         public function getActiveEmployees()
    {
        try {
            $employees = Employee::select(
                ...
                ->where('role_id', 4) // Only Layout Artists (role_id = 4)
                ->orderBy('status', 'desc') // Active first
                ->orderBy('fname', 'asc')   // Then alphabetically
                ->get();
    ...
    public function assignJobOrder(Request $request)
    {
        ...
        try {
            ...
            // Store first selected employee in assigned_to
