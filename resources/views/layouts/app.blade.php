...
/* 🟡 YELLOW Hover effect (changed from sky blue) */
.sidebar-collapsed button[onclick^="toggleDropdown"]:hover {
    ...
    padding-left: -3px;
    padding-right: 4px;
    color: #eab308; /* yellow-600 text color on hover */
}
...
/* 🟡 YELLOW Hover effect for the link background (changed from sky blue) */
.sidebar-collapsed .nav-link:hover {
    background-color: #fef08a; /* yellow-100 */
    color: #eab308; /* yellow-600 text color */
...
/* 🟡 ACTIVE PAGE INDICATOR - Yellow highlight for current page */
.nav-link.active,
...
/* Active state in expanded sidebar */
...
<nav class="flex-1 overflow-y-auto p-4 custom-scrollbar">
    <ul class="space-y-2">
        ...
        <li class="relative">
            <button
                ...
                <svg id="arrow-inventoryDropdown"
                     ...
                     {{ request()->routeIs('stock') || request()->routeIs('outstock') || request()->routeIs('stockadjustment') ? 'rotate-180' : '' }}"
            ...
            <ul id="inventoryDropdown"
                onmouseenter="preventDropdownClose('inventoryDropdown')"
                onmouseleave="allowDropdownClose('inventoryDropdown')"
                class="{{ request()->routeIs('stock') || request()->routeIs('outstock') || request()->routeIs('stockadjustment') ? 'block' : 'hidden' }} 
                ...
                <li>
                    <a href="{{ route('stockadjustment') }}"
                       class="block px-4 py-2 flex items-center space-x-2 rounded-lg text-sm font-medium transition duration-150
                       {{ request()->routeIs('stockadjustment') 
                            ? 'bg-yellow-100 text-yellow-700 shadow-md hover:bg-yellow-200' 
                            : 'text-gray-800 hover:bg-yellow-100 hover:text-yellow-700' }}">
        ...
        <li class="relative">
            <button
                ...
                class="dropdown-button w-full flex items-center justify-between p-3 rounded-lg text-sm font-medium 
                transition duration-150 text-gray-800 hover:bg-yellow-100 hover:text-yellow-700">
                <div class="flex items-center space-x-3">
            ...
            <ul id="managementDropdown"
                onmouseenter="preventDropdownClose('managementDropdown')"
                onmouseleave="allowDropdownClose('managementDropdown')"
                class="{{ request()->routeIs('supplier') || request()->routeIs('employee') || request()->routeIs('customer') || request()->routeIs('role') ? 'block' : 'hidden' }} 
                       mt-1 bg-white rounded-lg shadow-lg overflow-hidden text-sm">
                <li>
                    <a href="{{ route('supplier') }}"
                       ...
                       {{ request()->routeIs('supplier') 
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg"
                            ...
                            <path d="M13 14l4-2 4 2v4l-4 2-4-2v-4z" />
                            <path d="M13 14l4 2 4-2" />
                        </svg>
                        <span>Suppliers</span>
                    </a>
                </li>
      ...
      <!-- 🚪 Fixed Logout Button -->
        <div class="p-4 border-t border-gray-200 bg-white sticky bottom-0">
           <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit"
                    class="logout-link flex items-center space-x-3 p-3 rounded-lg text-sm font-semibold text-white bg-gray-800 hover:bg-yellow-500 hover:text-black transition duration-150 w-full text-left">
    ...
    <div class="flex-1 flex flex-col max-h-screen overflow-hidden">
       ...
       <!-- Top Bar -->
<header class="bg-yellow-300 p-[1.23rem] shadow-md flex justify-between items-center z-10 sticky top-0">
            
            <!-- Sidebar Toggle Button and Title -->
            <div class="flex items-center space-x-4">
                <button id="sidebarToggle" 
                    class="text-gray-500 hover:text-yellow-400 transition duration-150 focus:outline-none p-2 rounded-md hover:bg-yellow-50">
...
function preventDropdownClose(id) {
    preventClose[id] = true;
}

function allowDropdownClose(id) {
    preventClose[id] = false;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ✅ Auto-close dropdowns when clicking outside (respect preventClose)
    document.addEventListener('click', (event) => {
        const isDropdownButton = event.target.closest('button[onclick^="toggleDropdown"]');
        const isInsideDropdown = dropdownIds.some(id => {
            const dropdown = document.getElementById(id);
        ...
        if (!isDropdownButton && !isInsideDropdown) {
            dropdownIds.forEach(id => {
                if (preventClose[id]) return; // do not close if hovered
                const dropdown = document.getElementById(id);
                const arrow = document.getElementById(`arrow-${id}`);
                if (dropdown && arrow) {
                    dropdown.classList.add('hidden');
