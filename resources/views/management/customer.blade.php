...
<div x-data="customerPage()" class="px-4 sm:px-6 lg:px-8">
    ...
    <div class="max-w-7xl mx-auto mb-6 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div class="w-full md:w-auto flex items-center justify-end space-x-3">
            @include('added.archive_customer')
...
<script>
function archiveCustomer(button) {
    ...
    fetch('{{ route("customers.archive", ":id") }}'.replace(':id', id), {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function unarchiveCustomer(id) {
    if (!confirm('Remove this customer from archive?')) return;

    fetch('{{ route("customers.unarchive", ":id") }}'.replace(':id', id), {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function deleteCustomerPermanently(id) {
