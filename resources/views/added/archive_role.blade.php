<div x-data="{
        ...
        filterArchived() {
            ...
            const rows = document.querySelectorAll('#archiveRolesBody tr[data-role]');
            ...
            const empty = document.getElementById('archiveRolesEmpty');
            if (empty) empty.style.display = hasVisible ? 'none' : '';
        }
    }"
>
    {{-- Top-right Archive button (same as suppliers) --}}
    ...
    <div x-show="showArchiveModal" x-transition x-cloak
        ...
        <div @click.away="showArchiveModal = false"
            ...
            <h2 class="text-2xl font-bold mb-6 text-gray-800 border-b pb-3 flex items-center justify-between">
                <span>Archive Roles</span>
                <input
                    type="text"
                    x-model="searchQuery"
                    @input="filterArchived()"
                    placeholder="Search archived roles"
                    class="ml-4 w-64 px-3 py-1.5 text-sm border border-gray-300 rounded-full
                ...
                <table class="min-w-full divide-y divide-gray-200 border border-gray-100">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase text-gray-600">Role Name</th>
                    ...
                    <tbody id="archiveRolesBody" class="divide-y divide-gray-100">
                        @forelse($roles->where('archive', 'Archived') as $role)
                            <tr
                                data-role
                                data-id="{{ $role->role_id }}"
                                data-search="{{ $role->role_name }} {{ $role->description }}"
                                ...
                                <td class="px-4 py-2 text-center text-sm text-gray-700">
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        <button
                                            ...
                                            onclick="unarchiveRole({{ $role->role_id }})"
            ...
            <div class="mt-8 flex justify-end">
                <button @click="showArchiveModal = false"
                        class="bg-yellow-500 text-gray-900 font-bold px-8 py-2 rounded-full
