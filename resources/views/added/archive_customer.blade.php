<div x-data="{
        ...
        filterArchived() {
            ...
            const rows = document.querySelectorAll('#archiveCustomersBody tr[data-customer]');
            ...
            const empty = document.getElementById('archiveCustomersEmpty');
               ...
               <table class="min-w-full divide-y divide-gray-200 border border-gray-100">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase text-gray-600">First Name</th>
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase text-gray-600">Middle Name</th>
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase text-gray-600">Last Name</th>
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase text-gray-600">Contact No.</th>
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase text-gray-600">Address</th>
                            <th class="px-4 py-3 text-center text-xs font-bold uppercase text-gray-600">Actions</th>
                        </tr>
                    </thead>
                    <tbody id="archiveCustomersBody" class="divide-y divide-gray-100">
                        @forelse($customers->where('archive', 'Archived') as $customer)
                            <tr
                                data-customer
                                data-id="{{ $customer->customer_id }}"
                                data-search="{{ $customer->fname }} {{ $customer->mname }} {{ $customer->lname }} {{ $customer->contact_no }} {{ $customer->address }}"
                            ...
                            >
                                ...
                                <td class="px-4 py-2 text-left text-sm text-gray-700 max-w-xs truncate">
                                    {{ $customer->address }}
                                </td>
                                <td class="px-4 py-2 text-center text-sm text-gray-700">
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        <button
                                            ...
                                            onclick="unarchiveCustomer({{ $customer->customer_id }})"
                                        ...
                                        <button
                                            ...
                                            onclick="deleteCustomerPermanently({{ $customer->customer_id }})"
