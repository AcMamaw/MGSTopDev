<div x-data="{
        ...
        filterArchived() {
            ...
            const rows = document.querySelectorAll('#archiveCategoriesBody tr[data-category]');
            ...
            const empty = document.getElementById('archiveCategoriesEmpty');
            if (empty) empty.style.display = hasVisible ? 'none' : '';
        }
    }"
>
    <div class="w-full flex justify-end">
        {{-- Trigger button (same style as Delivery History) --}}
                ...
                <table class="min-w-full divide-y divide-gray-200 border border-gray-100">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase text-gray-600">Category Name</th>
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase text-gray-600">Description</th>
                            <th class="px-4 py-3 text-center text-xs font-bold uppercase text-gray-600">Actions</th>
                        </tr>
                    </thead>
                    <tbody id="archiveCategoriesBody" class="divide-y divide-gray-100">
                       @forelse ($categories->where('archive', 'Archived') as $category)
                            <tr
                                data-category
                                data-id="{{ $category->category_id }}"
                                data-search="{{ $category->category_name }} {{ $category->description }}"
                            ...
                            >
                                ...
                                <td class="px-4 py-2 text-left text-sm text-gray-700 max-w-md truncate">
                                ...
                                <td class="px-4 py-2 text-center text-sm text-gray-700">
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        <button
                                            ...
                                            onclick="unarchiveCategory({{ $category->category_id }})"
                                        ...
                                        <button
                                            ...
                                            onclick="deleteCategoryPermanently({{ $category->category_id }})"
...
function unarchiveCategory(id) {
    if (!confirm('Remove this category from archive?')) return;

    fetch('{{ route("categories.unarchive", ":id") }}'.replace(':id', id), {
        method: 'PUT',
        headers: {
    ...
    .then(res => res.json())
    .then(() => {
        // reload the whole page so the row disappears from archive and reappears in main table
        window.location.reload();
    });
}

function deleteCategoryPermanently(id) {
