<div x-data="{
        ...
        filterArchived() {
            ...
            const rows = document.querySelectorAll('#archiveProductsBody tr[data-product]');
            ...
            const empty = document.getElementById('archiveProductsEmpty');
                ...
                <table class="min-w-full divide-y divide-gray-200 border border-gray-100">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase text-gray-600">Product Name</th>
                    ...
                    <tbody id="archiveProductsBody" class="divide-y divide-gray-100">
                       @forelse ($products->where('archive', 'Archived') as $product)
                            <tr
                                data-product
                                data-id="{{ $product->product_id }}"
                                data-search="{{ $product->product_name }} {{ $product->description }}"
                                ...
                                <td class="px-4 py-2 text-center text-sm text-gray-700">
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        <button
                                            ...
                                            onclick="unarchiveProduct({{ $product->product_id }})"
                                        ...
                                        <button
                                            ...
                                            onclick="deleteProductPermanently({{ $product->product_id }})"
...
function unarchiveProduct(id) {
    if (!confirm('Remove this product from archive?')) return;

    fetch('{{ route("products.unarchive", ":id") }}'.replace(':id', id), {
        method: 'PUT',
        headers: {
